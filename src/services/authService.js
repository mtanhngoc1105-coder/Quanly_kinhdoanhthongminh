...
export const loginApi = async (
  ...
  try {
    ...
    // Cho phép đăng nhập "Người mua" bằng bất kỳ tài khoản/mật khẩu nào
    if (!user && role === "user") {
      user = {
        id: "customer-" + Date.now(),
        name: email === "customer@smartfood.com" ? "Khách Hàng VIP" : (email.split('@')[0] || "Khách hàng"),
        email: email,
        roleId: 1,
        role: "user",
        isActive: true,
    ...
    try {
      ...
      // Cho phép đăng nhập "Người mua" bằng bất kỳ tài khoản/mật khẩu nào (offline fallback)
      if (!user && role === "user") {
        user = {
          id: "customer-" + Date.now(),
          name: email === "customer@smartfood.com" ? "Khách Hàng VIP" : (email.split('@')[0] || "Khách hàng"),
